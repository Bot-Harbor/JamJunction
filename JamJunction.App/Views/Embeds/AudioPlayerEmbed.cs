...
public class AudioPlayerEmbed
{
    ...
    /// <summary>
    /// Builds an embed suggesting a track for the user to play next, generated by
    /// the AI song recommender after a track finishes playing.
    /// </summary>
    /// <param name="basedOn">
    /// The <see cref="LavalinkTrack"/> that just finished playing and that the
    ...
    /// </param>
    /// <param name="reason">
    /// A short explanation of why the song was recommended.
    /// </param>
    /// <returns>
    /// A <see cref="DiscordMessageBuilder"/> presenting the AI recommendation
    /// along with a button that queues the suggested track.
    /// </returns>
    public DiscordMessageBuilder SongRecommendation(LavalinkTrack basedOn, string title, string artist, string reason)
    {
        var query = Uri.EscapeDataString($"{title} {artist}");

        var uri = basedOn.Uri?.ToString().ToLower() ?? "";
        ...
        else if (uri.Contains("music.youtube.com"))
        {
            platformName = "Platform: YouTube Music";
            platformIcon =
                "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/Youtube_Music_icon.svg/960px-Youtube_Music_icon.svg.png";
        ...
        else if (uri.Contains("deezer.com"))
        {
            platformName = "Platform: Deezer";
            platformIcon =
                "https://companieslogo.com/img/orig/DEEZR.PA-dbdcf2cf.png?t=1721547851";
        ...
        {
            platformName = "Platform: SoundCloud";
            platformIcon = "https://cdn-icons-png.flaticon.com/512/145/145809.png";
            searchUrl = $"https://soundcloud.com/search?q={query}";
        }
        else if (uri.Contains("music.apple.com"))
        {
            platformName = "Platform: Apple Music";
            platformIcon =
                "https://upload.wikimedia.org/wikipedia/commons/thumb/5/5f/Apple_Music_icon.svg/500px-Apple_Music_icon.svg.png";
            searchUrl = $"https://music.apple.com/us/search?term={query}";
        }
        else
        {
            platformName = "Platform: Spotify";
        ...
        var embed = new DiscordEmbedBuilder
        {
            ...
            Description =
                $"Because you listened to **{basedOn.Title}** by **{basedOn.Author}**, " +
                "you might enjoy:\n\n" +
                $"🎵  •  [**{title}**]({searchUrl}) by **{artist}**\n\n" +
                $"_{reason}_",
        ...
        if (basedOn.ArtworkUri != null)
            embed.Thumbnail = new DiscordEmbedBuilder.EmbedThumbnail
            {
                Url = basedOn.ArtworkUri.AbsoluteUri
            };
