...
/// <summary>
/// Handles the pause button interaction for the audio player.
/// </summary>
/// <remarks>
/// This event is triggered when a user presses the pause button on the
/// player interface. It validates the user's voice channel, retrieves
/// the Lavalink player instance, pauses the currently playing track,
/// and updates the player embed message.
/// </remarks>
public class PauseButtonEvent : IButton
{
    /// <summary>
    ...
    public PauseButtonEvent(IAudioService audioService, DiscordClient discordClient)
    {
    ...
    /// <summary>
    /// Gets or sets the voice channel that the user is currently connected to.
    /// </summary>
    /// <remarks>
    /// This property is used to validate that the user issuing the pause
    /// interaction is in the same voice channel as the bot.
    /// </remarks>
    private DiscordChannel UserVoiceChannel { get; set; }

    /// <summary>
    /// Executes the pause button interaction logic.
    /// </summary>
    /// <param name="sender">
    /// The <see cref="DiscordClient"/> instance that triggered the interaction.
    /// </param>
    /// <param name="btnInteractionArgs">
    /// The interaction event arguments containing the button data,
    /// user information, and guild context.
    /// </param>
    /// <returns>
    /// A <see cref="Task"/> representing the asynchronous operation.
    /// </returns>
    /// <remarks>
    /// This method performs several validation checks before pausing playback:
    ...
    ///
    /// If all validations pass, the current track is paused and the
    /// player interface embed is updated.
    /// </remarks>
    public async Task Execute(DiscordClient sender, ComponentInteractionCreateEventArgs btnInteractionArgs)
    {
        if (btnInteractionArgs.Interaction.Data.CustomId == "pause")
        {
            ...
            var pauseMessage = await channel.CreateFollowupMessageAsync(
                audioPlayerEmbed.Pause());

            await Task.Delay(10000);
            _ = channel.DeleteFollowupMessageAsync(pauseMessage.Id);
        }
    }
}