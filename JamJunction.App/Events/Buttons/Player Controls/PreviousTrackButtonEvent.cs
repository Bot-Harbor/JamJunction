...
/// <summary>
/// Handles the previous track button interaction for the audio player.
/// </summary>
/// <remarks>
/// This event is triggered when a user presses the previous track button on the
/// player interface. The handler validates the user's voice channel, retrieves
/// the Lavalink player, reinserts the current track at the front of the queue,
/// and begins playing the most recent track from the playback history.
/// </remarks>
public class PreviousTrackButtonEvent : IButton
{
    /// <summary>
    ...
    public PreviousTrackButtonEvent(IAudioService audioService, DiscordClient discordClient)
    {
    ...
    /// <summary>
    /// Executes the previous track button interaction logic.
    /// </summary>
    /// <param name="sender">
    ...
    {
        if (btnInteractionArgs.Interaction.Data.CustomId == "previous-track")
        {
            ...
            {
                var errorMessage = await channel.CreateFollowupMessageAsync(
                    errorEmbed.NoPreviousTrackError());
                await Task.Delay(10000);
                _ = channel.DeleteFollowupMessageAsync(errorMessage.Id);
                return;
            }
            ...
            var previousMessage = await channel.CreateFollowupMessageAsync(
                new DiscordFollowupMessageBuilder().AddEmbed(
                    audioPlayerEmbed.PreviousTrack()));

            await Task.Delay(10000);
            _ = channel.DeleteFollowupMessageAsync(previousMessage.Id);
        }
    }
}
